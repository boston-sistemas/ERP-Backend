...
def import_models():
    ...
    from mecsa_erp.area_operaciones.modulo1.models import (
        OrdenServicioTejeduriaDetalleReporteLog,
    )
...
def initialize_args():
    ...
    parser_generate_sql = subparsers.add_parser(
        "generate_sql", help="Generate SQL create tables"
    )
    parser_generate_sql.add_argument(
        "-o", "--output", help="Output file name", default="", required=False
    )
    parser_generate_sql.add_argument(
        "--dialect",
