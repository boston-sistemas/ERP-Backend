...
class PDFService:
    ...
    @staticmethod
    def on_first_page(canvas, doc, data, image_path, reduction_percentage, cant_page):
        PDFService.draw_background(canvas, doc)
        PDFService.draw_image(canvas, doc, image_path, reduction_percentage)
        PDFService.draw_signature(canvas, doc)
        PDFService.draw_comments_section(canvas, doc, data["comments"], cant_page)

    @staticmethod
    def on_later_pages(canvas, doc, data, cant_page):
        PDFService.draw_background(canvas, doc)
        PDFService.draw_signature(canvas, doc)
        PDFService.draw_comments_section(canvas, doc, data["comments"], cant_page)
    ...
    def generate_pdf(
        ...
        styles.add(ParagraphStyle(name="Roboto", fontName="Roboto", fontSize=10))
        styles.add(
            ParagraphStyle(name="Roboto-Bold", fontName="Roboto-Bold", fontSize=17)
        )
        styles.add(
            ParagraphStyle(
        ...
        titulo = "<font size=17><b>Programación Tintorería Semana:</b> <b>{0}</b></font>".format(
            data["semana"]
        )
        ...
        table = self.draw_table_header(valor, pagesize)

        elements.append(table)
        elements.append(Spacer(1, 24))

        titles = [
            Paragraph(title, styles["Roboto-Bold-Small"]) for title in data["title"]
        ]
        values = [
            [Paragraph(i, styles["Roboto-Small"]) for i in row]
        ...
        pdf.build(
            elements,
            onFirstPage=lambda canvas, doc: PDFService.on_first_page(
                canvas, doc, data, image_path, reduction_percentage, cant_page
            ),
            onLaterPages=lambda canvas, doc: PDFService.on_later_pages(
