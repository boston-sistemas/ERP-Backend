...
from src.core.constants import ID_COLOR_CRUD, SERVICE_CODE_SUPPLIER_WEAVING
from src.core.repository import RateRepository
from src.operations.models import ServiceRate


class WeavingRateRepository(RateRepository):
    def __init__(self, db: AsyncSession, flush: bool = False) -> None:
        super().__init__(ServiceRate, db, flush)

    @staticmethod
    def get_weaving_rate_fields() -> tuple:
    ...
    def get_load_options(self) -> list[Load]:
        options: list[Load] = []
        options.append(load_only(*self.get_weaving_rate_fields()))

        return options

    async def find_weaving_rate_by_id(
        self,
        rate_id: str,
        ...
        weaving_rate = await self.find_rate_by_id(
            filter=filter, options=options, joins=joins
        )

        return weaving_rate

    async def find_weaving_rates(
        self,
        ...
        weaving_rates = await self.find_rates(
            filter=filter,
            options=options,
            limit=limit,
            offset=offset,
