...
class FabricService:
    ...
    async def _assign_recipe_to_fabrics(
        self, fabrics: list[InventoryItem], include_fabric_instance: bool = False
    ) -> None:
        if not fabrics:
            return None
    ...
    async def _validate_fabric_recipe(
        ...
        yarn_ids = {item.fabric_id for item in fabric_recipe}
        ...
        if len(fabric_recipe) != len(fabrics):
            return YARN_NOT_FOUND_IN_FABRIC_RECIPE_FAILURE

        if any(not fiber.is_active for fiber in fabrics):
    ...
    def _is_fabric_recipe_unique(
        ...
        _current_recipe = {(item.fabric_id, item.proportion) for item in current_recipe}
        recipe_size = len(_current_recipe)

        return not any(
            recipe_size == len(fabric.recipe)
            and _current_recipe
            == {(item.fabric_id, item.proportion) for item in fabric.recipe}
            for fabric in fabrics
        )
    ...
    async def _is_fabric_unique(
        ...
        **kwargs,
        ...
        _fabrics = await self.repository.find_fabrics(filter=filter)
        fabrics = (
            _fabrics
        ...
        await self._assign_recipe_to_fabrics(fabrics=_fabrics)
    ...
    async def _read_fabric(
        ...
        fabric = await self.repository.find_fabric_by_id(
            fabric_id=fabric_id, include_color=include_color
        )
        ...
        if include_recipe:
            await self._assign_recipe_to_fabrics([fabric], include_fabric_instance=True)
    ...
    async def read_fabrics(
        ...
        if include_recipe:
            await self._assign_recipe_to_fabrics(fabrics, include_fabric_instance=True)
    ...
    async def create_fabric(
        ...
        #     YarnFiber(fabric_id=fabric_id, **item.model_dump()) for item in form.recipe
        # )
