...
from mecsa_erp.usuarios.schemas.usuario import (
    RolCreateSchema, RolListSchema, RolSchema, RolUpdateSchema)
...
def get_rol(session: SessionDependency, rol_id: int):
    rol = crud_rol.get_by_pk_or_404(
        session, rol_id, [joinedload(Rol.accesos)]
    )
    return rol
...
@router.put("/{rol_id}")
def update_rol(
    session: SessionDependency, rol_id: int, rol: RolUpdateSchema
):
    ...
    message, _ = crud_rol.update(
        session, db_rol, rol.model_dump(exclude_unset=True)
    )
